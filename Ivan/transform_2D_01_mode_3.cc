...
void TransformAndDraw(const float* shape_coords) {
	...
	for (int i = 0; i < kNumberPoints; ++i) {
		esat::Vec3 result = { shape_coords[i * 2], shape_coords[i * 2 + 1], 1.0f };
		result = esat::Mat3TransformVec3(esat::Mat3Rotate(g_angle), result);
		result = esat::Mat3TransformVec3(esat::Mat3Scale(kRadius, kRadius), result);
		result = esat::Mat3TransformVec3(esat::Mat3Translate((float) esat::MousePositionX(),
		 																					 					 (float) esat::MousePositionY()), result);
