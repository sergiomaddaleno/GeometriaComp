...
void TransformAndDraw(const float* shape_coords) {
	...
	esat::Mat3 scale = esat::Mat3Scale(kRadius, kRadius);
	esat::Mat3 rotate = esat::Mat3Rotate(g_angle);
	esat::Mat3 translate = esat::Mat3Translate((float) esat::MousePositionX(),
	 																					 (float) esat::MousePositionY());
	esat::Mat3 m = esat::Mat3Multiply(rotate, scale);
	m = esat::Mat3Multiply(translate, m);
